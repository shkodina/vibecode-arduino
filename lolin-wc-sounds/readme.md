...
Источник wav: https://zvukogram.com/category/penie-ptits/ — конвертировать
...
- SD-модуль лучше кормить **3.3 В**, не 5 В. Многие китайские mini-reader
  якобы «5V», но линия данных у ESP — 3.3 В. Берём **3V3** с платы.
...
D6  (GPIO12) ------------- MISO
D5  (GPIO14) ------------- SCK
3V3 ---------------------- VCC   (именно 3.3 В)
GND ---------------------- GND
```

Почему так:

- I2S на ESP8266: BCLK=GPIO15, WS=GPIO2, DATA=GPIO3. Других легальных
  пинов нет.
- GPIO15 (D8) при старте должен быть LOW, иначе плата не загрузится.
  У MAX98357 вход BCLK обычно не тянет линию вверх — это ок. Если плата
  не стартует — поставить подтяжку 10 кОм с D8 на GND.
- RX (GPIO3) занят I2S DATA. USB-Serial при заливке всё равно работает,
  а `Serial` в скетче идёт через тот же UART0. После старта I2S лог в
  монитор порта может портиться — это ожидаемо. Отладка до `i2s_begin()`
  нормальная.
- SD на аппаратном SPI: D5/D6/D7, CS на D0. D0 нельзя использовать как
  interrupt, для CS это нормально.
- PIR на D2 — свободный GPIO без ролей загрузки.

Альтернатива, если SD-модуль капризный: тот же SPI, но CS на D1 (GPIO5).
Тогда в начале `.ino` поменять `PIN_SD_CS` с `D0` на `D1`. Остальное
не трогать.

### Как ESP8266 совмещает SD и I2S

Одновременно гонять SPI и I2S на этой плате тяжело. Прошивка делает
**двойной буфер по 4 КБ**:

1. читает кусок WAV с карты в буфер A;
2. пока A играет в I2S, читает следующий кусок в буфер B;
3. меняет их местами.
