...
```json
{
  ...
  "motion": {
    ...
    "cooldown_seconds": 5,
    "repeat_seconds": 15,
    "idle_repeat_seconds": 15,
    "stable_ms": 400,
    "boot_ignore_seconds": 10
  },
  "playback": {
    "schedule": [
      {
...
| Поле                    | Смысл |
| ----------------------- | ----- |
| `timeout_seconds`       | сколько играть после последнего **принятого** движения |
| `cooldown_seconds`      | пауза перед новым стартом play |
| `repeat_seconds`        | пока **играет**: импульс PIR раньше этого не продлевает таймер (повтор Delay). `0` = выкл |
| `idle_repeat_seconds`   | в **тишине**: импульс раньше этого не включает звук. Чуть больше цикла Delay в пустой комнате, иначе шум снова запустит play. `0` = выкл |
| `stable_ms`             | спокойный HIGH, миллисекунды: отсечь дребезг пина |
| `boot_ignore_seconds`   | после ребута столько секунд PIR молчит |


`repeat_seconds` / `idle_repeat_seconds` — фильтр ложных циклов HC-SR501
(игра / ожидание). `timeout_seconds` — сколько секунд музыки после
принятого захода.

В `GET /api/status`: `motion` — уже отфильтрованное движение (им оно
продлевает музыку), `motion_raw` — текущий уровень пина. Если `motion_raw`
мигает, а `motion` нет — это как раз ложные импульсы. Если оба долго `true`
без человека — крутилка чувствительности PIR слишком большая, или модуль
ловит радио ESP.
...
В yaml те же поля `motion`, что в json. Править удобнее yaml, на карту
класть json:
...
полная тишина в ожидании движения. Если не играет — `0`. Пока `motion` true,
это примерно `motion.timeout_seconds` (отсчёт начнётся, когда PIR отпустит
и продержится LOW). Короткие ложные HIGH таймер не сбрасывают. Если
