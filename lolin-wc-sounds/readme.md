...
| `repeat_seconds`        | импульс PIR раньше этого = повтор Delay модуля, не человек. `0` = выкл. В ванной с длинной крутилкой Delay поставь 45–60 |
| `stable_ms`             | спокойный HIGH, миллисекунды: отсечь дребезг пина |
| `boot_ignore_seconds`   | после ребута столько секунд PIR молчит |


`repeat_seconds` не путать с `timeout_seconds`: первое — фильтр ложных циклов HC-SR501, второе — сколько секунд музыки после реального захода.
