...
`motion.timeout_seconds` — тишина после последнего **подтверждённого**
движения. HIGH считается движением, если после последнего фронта пин
спокойно держится ≥400 мс (этот экземпляр PIR держит импульс ~1.5–2 с,
на фронте ещё дребезг). Короткие выбросы наводки так отсекаются. Первые
10 с после ребута PIR игнорируется. `motion.cooldown_seconds` — пауза
перед новым play.
