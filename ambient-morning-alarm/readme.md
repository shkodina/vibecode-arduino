...
- WiFi и BLE вместе на C3 не живут. При старте читаются перемычки:
  GPIO5→GND = только BLE, GPIO6→GND = только WiFi, иначе WiFi.
  Поле статуса `radioMode`: `"wifi"` / `"ble"`.
...
- 00.00.007 ребутилась: `setupHttp` / `WebServer.begin` до `WiFi.mode`, lwIP
  mutex NULL, `assert xQueueSemaphoreTake`. Сначала `WiFi.mode(WIFI_STA)` без
  associate, `WiFi.begin` по-прежнему через 3 с.

Что сделано в 00.00.007:

- холодный старт с БП: radio не стартует в `setup`. WiFi через 3 с, BLE через 6 с,
  TX 8.5 dBm. Файл `early_boot.cpp`: brownout выключен, GPIO4 LOW в
  ctor/`initVariant` (до Arduino setup). Иначе POWERON: WiFi+BLE вместе
  роняют 3.3 В, ресет-цикл, лента горит; RESET (питание уже стабильно)
  заводил плату.

Что сделано в 00.00.006:

- GPIO4 (SIG MOSFET) сразу OUTPUT LOW в первой строке `setup`, до Serial/BLE —
  иначе при питании от БП лента горит, пока пин Hi-Z;
- `BLEDevice::init` убран из `setup()`: он блокировал `loop()` / HTTP и мог
  не уложиться в watchdog при холодном старте с БП. BLE стартует в задаче
  `bleinit` (стек 16 КБ), веб кормится в `loop` сразу.
...
PWM: GPIO4 (на шёлке платы «4») → `SIG` MOSFET (LR7843). 500 Гц, 12 бит,
гамма 2.0 и потолок duty 12%: глаз на LR7843 насыщается к ~10% линейного
ШИМ, поэтому 100% в вебе — это ~12% импульса, а не полный открытый MOSFET.
Встроенный синий LED платы (GPIO8, active LOW) зеркалит яркость: если тест
идёт, синий тоже горит, даже без ленты.

Режим радио (INPUT_PULLUP, замыкание на GND **до/в момент включения**):

```text
GPIO5 ──► GND   только BLE (приложение), WiFi выключен
GPIO6 ──► GND   только WiFi (веб), BLE выключен
ничего / обе    только WiFi
```

Нельзя сажать на землю GPIO8 (LED/strapping) и GPIO9 (BOOT).
...
1. Ещё до `setup`: brownout выключен (регистр), GPIO4 LOW в `initVariant`.
2. Сразу GPIO4 = LOW (MOSFET закрыт) и синий LED выключен.
3. Watchdog на заводские 30 с.
4. PWM, NVS (WiFi, будильники, таймер, watchdog), перенастройка WDT.
...
Яркость — проценты `0..100` воспринимаемого света (гамма ШИМ, не duty).
Для ramp/pulse финиш не может быть меньше старта.
...
- Android APK с 1.1.0 знает `glowSeconds` / `fadeSeconds` / `strobe` и
  `setAlarm`. Старый APK 1.0.0 при полном `setSettings` сотрёт новые поля
  pulse и запишет неизвестный `type` как `ramp`.
- Часовой пояс зашит `GMT+3`.
- Кривая ШИМ: `LED_PWM_GAMMA` и `LED_PWM_VISUAL_DUTY` в `config.h`.
  100% в вебе ≈ 12% электрического duty (дальше глаз на этой ленте не
  отличает). Если 40–100% всё ещё плоские — чуть поднять потолок
  (0.15…0.20). Если низ яркий — поднять гамму.
