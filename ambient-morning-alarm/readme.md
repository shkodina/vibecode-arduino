...
Текущая прошивка: **00.00.003**. BLE-имя: `piper-light-alarm-0000003`.

Что сделано в 00.00.003 (ещё не прошивалось в этой сессии, только код):
...
- `LightModeConfig` / `computeLightBrightness` — цикл ramp/pulse;
...
6. Рекламирует BLE-имя из версии, сейчас `piper-light-alarm-0000003`.
...
Яркость — проценты `0..100`. Финиш не может быть меньше старта.
`rampSeconds` и `totalSeconds` всегда `> 0`.
...
- WiFi SSID/пароль;
- 10 будильников: время, дни, режим, поля pulse включая свечение и затухание.
...
- Имя: `piper-light-alarm-` + major/minor/patch без точек. Для `00.00.003`
  это `piper-light-alarm-0000003`
- PIN / passkey: `8888`
...
1. Serial: версия `00.00.003` и BLE-имя `piper-light-alarm-0000003`.
2. WiFi IP в логе.
...
5. Тест `pulse` со свечением и затуханием, затем стоп.
6. BLE scanner: `piper-light-alarm-0000003`.

## Известные ограничения

- Прошивка 00.00.003 залита 2026-09-12 на COM5 (esptool, Hash verified).
- Android APK ещё без полей `glowSeconds` / `fadeSeconds`. Если приложение
  сделает полный `setSettings` со старой схемой, прошивка запишет эти поля
  как 0 и сотрёт свечение/затухание.
...
- подтянуть Android под новые поля pulse и `setAlarm` / `nextAlarm`
