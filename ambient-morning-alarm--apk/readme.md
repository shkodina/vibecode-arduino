...
- показывает статус, ближайший будильник (`nextAlarm`, как баннер веба),
  активный запуск и кнопку «Остановить»;
- после подключения кнопка «Подключить» меняется на «Обновить»;
- экраны: **Будильники**, **Таймер**, **Тест**, **Настройки**;
...
- команды: `getSettings`, `setSettings`, `setAlarm`, `setTimer`, `getWifi`,
  ...
  `stopTest`. После connect приложение шлёт время телефона (`setTime` /
  unix epoch UTC), затем `getSettings`.
- таймер и «Сохранить все» не шлют полный `setSettings` по BLE (пакет не
  влезает в MTU и даёт Operation rejected). Пишутся `setAlarm` по одному и
  `setTimer`. Старт таймера: `setTimer` + `startTimer`.
- BLE: таймаут 20 с и 3 ретрая команды (на вкладке «Настройки»). Баннер
  только когда все ретраи и переподключения кончились.
- keepalive каждые 8 с: `getStatus` или тихий reconnect.
- если время контроллера уехало больше чем на 180 с — `setTime`.
- при потере связи шапка продолжает показывать остаток и фазу свечения
  (розжиг / свечение / затухание / темнота / строб) по последнему статусу.
- изменённые от завода параметры приложения пишутся в AsyncStorage.
...
Пакет: `local.piper.ambientmorningalarm`. Версия приложения **1.3.0**
(`versionCode` 4).
...
5. Нажми «Подключить» — приложение найдёт `piper-light-alarm-*`. Если уже
   подключено, та же кнопка станет «Обновить» и перечитает настройки.
6. Если телефон спросит PIN BLE — не нужен (Just Works с 00.00.013). Старые
   сопряжения `piper-light-alarm-*` лучше забыть.
7. Для загрузки всех будильников и сохранения таймера лучше прошивка
   **00.00.016** (куски BLE 96 байт, команда `setTimer`).
